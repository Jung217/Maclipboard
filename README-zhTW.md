...
- **全域快速鍵存取**：在任何應用程式中立即喚出剪貼簿歷史。預設為 <kbd>CONTROL (⌃)</kbd> + <kbd>V</kbd>，但可以在設定中錄製並修改為任何按鍵組合！
- **鍵盤驅動的自動貼上**：使用 <kbd>UP (↑)</kbd> 與 <kbd>DOWN (↓)</kbd> 方向鍵瀏覽歷史紀錄並按下 <kbd>RETURN (⏎)</kbd>，或直接點擊項目，即可自動貼上至您當前的活動視窗。
- **分頁切換**：使用 <kbd>LEFT (←)</kbd> 與 <kbd>RIGHT (→)</kbd> 方向鍵，即可快速在 `All`（所有）歷史紀錄、`Pinned`（釘選）項目、`Images`（圖片）或 `Files`（檔案）之間進行切換。
- **進階操作快捷鍵**：使用方向鍵選取項目時：
  - <kbd>SPACE</kbd>：開啟該項目的完整純文字預覽視窗。
  - <kbd>CONTROL (⌃)</kbd> + <kbd>P</kbd>：直接透過鍵盤切換該項目的釘選狀態。
  - <kbd>COMMAND (⌘)</kbd> + <kbd>BACKSPACE (⌫)</kbd>：將該項目從歷史紀錄中永久刪除。
  - <kbd>COMMAND (⌘)</kbd> + <kbd>SHIFT (⇧)</kbd> + <kbd>BACKSPACE (⌫)</kbd>：立即清除所有未被釘選的項目。
- **豐富的媒體支援**：Maclipboard 不僅僅支援文字！
  - **檔案**：從 Finder 中直接複製檔案與資料夾（透過路徑），顯示原生文件圖示，並允許您將其自動貼上至任何地方。
  - **原生螢幕快照**：在背景靜默監控您指定的螢幕快照資料夾（預設為 `~/Desktop`）。當您使用 macOS 原生截圖捷徑（<kbd>COMMAND (⌘)</kbd> + <kbd>SHIFT (⇧)</kbd> + <kbd>4</kbd> 或 <kbd>COMMAND (⌘)</kbd> + <kbd>SHIFT (⇧)</kbd> + <kbd>5</kbd>）時，Maclipboard 會攔截新圖片並立即加入到剪貼簿歷史中！
- **豐富的自訂選項**：點擊面板頂部的「齒輪」圖示以存取原生設定：
  ...
  - **自訂螢幕快照資料夾**：更改 Maclipboard 監控螢幕快照檔案的資料夾。
  - **行為控制**：啟用「登入時啟動（Launch at Login）」以自動執行，或開啟「離開時清除紀錄（Clear History on Quit）」以保護隱私。

## 🆕 最新更新

- **打字體驗優化**：現在喚醒 Maclipboard 面板時將不再奪走其他應用程式（如 Google Chrome）的文字輸入焦點，讓您可以流暢打字。同時也穩定了空列表提示游標（`︱`）的閃爍動畫。
- **介面操作修正**：修復了點擊面板背景或頂部工具列空白處時，會意外觸發並開啟 macOS 原生「色彩挑選面板（Color Picker）」的錯誤。
...
此外，為提供「原生螢幕快照」支援（<kbd>COMMAND (⌘)</kbd> + <kbd>SHIFT (⇧)</kbd> + <kbd>4</kbd> / <kbd>COMMAND (⌘)</kbd> + <kbd>SHIFT (⇧)</kbd> + <kbd>5</kbd>），Maclipboard 需要 **「桌面（Desktop）資料夾」的存取權限**。這樣當 macOS 在您的桌面上產生新的截圖時，Maclipboard 就能偵測並將其無縫讀入到您的剪貼簿歷史中。
